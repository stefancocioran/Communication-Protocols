...
int receive_and_send(int connfd1, int connfd2)
{
    ...
    DIE(bytes_received == -1, "Receptionarea mesajului de la connfd1 a esuat");
    ...
    DIE(bytes_send == -1, "Trimiteterea mesajului catre connfd2 a esuat");;
...
void run_echo_server(int listenfd)
{
    ...
    connfd = listen(listenfd, 1);
    DIE(connfd == -1, "Eroare la ascultare");

    // TODO 5: Acceptati o conexiune
    connfd = accept(listenfd, (struct sockaddr*) &client_addr, &socket_len);
    DIE(connfd == -1, "Eroare la acceptarea conexiunii");
...
int main(int argc, char* argv[])
{
    ...
    DIE(listenfd == -1, "Eroare creare socket");
    ...
    DIE(b == -1, "Eroare la legarea socket-ului");
