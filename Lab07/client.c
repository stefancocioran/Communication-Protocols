...
void run_client(int sockfd) {
    ...
    while (read(STDIN_FILENO, buf, BUFLEN - 1) > 0 && !isspace(buf[0])) {
        ...
        DIE(bytes_send == -1, "Trimiteterea mesajului catre server a esuat");
        ...
        DIE(bytes_received == -1, "Receptionarea mesajului venit de la server a esuat");
...
int main(int argc, char* argv[])
{
    ...
    DIE(sockfd < 0, "Eroare la crearea socket-ului");
    ...
    DIE(c == -1, "Conexiunea catre server a esuat");
